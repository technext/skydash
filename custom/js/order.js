...
$(document).ready(function () {
	...
	if (divRequest == 'add') {
		...
		$("#createOrderForm").unbind('submit').bind('submit', function () {
			...
			var clientContact = $("#clientIC").val();
			var clientIC = $("#clientName").val();
			var paid = $("#paid").val();
			...
			if (clientIC == "") {
				$("#clientIC").after('<p class="text-danger"> Client IC field is required </p>');
				$('#clientIC').closest('.form-group').addClass('has-error');
			} else {
				$('#clientIC').closest('.form-group').addClass('has-success');
			} // /else
			...
			if (paymentStatus == "") {
				$("#paymentStatus").after('<p class="text-danger"> Payment Status field is required </p>');
			...
			if (paymentPlace == "") {
				$("#paymentPlace").after('<p class="text-danger"> Payment Place field is required </p>');
						...
						success: function (response) {
							...
							if (response.success == true) {
								...
								$(".success-messages").html('<div class="alert alert-success">' +
									...
									' <br /> <br /> <a type="text" onclick="printOrder(' + response.order_id + ')" class="btn btn-default"> <i class="glyphicon glyphicon-print"></i> Print </a>' +
		...
		$("#editOrderForm").unbind('submit').bind('submit', function () {
			...
			if (orderDate && clientName && clientContact && clientIC && paid && discount && paymentType && paymentStatus) {
				if (validateProduct == true && validateQuantity == true) {
					// create order button
					// $("#createOrderBtn").button('loading');
	...
	$.ajax({
		...
		success: function (response) {
			...
			tr += '</select>' +
				...
				'<td>' +
				'<div class="form-group">' +
				'<button class="btn btn-danger removeProductRowBtn" type="button" onclick="removeProductRow(' + count + ')"><i class="glyphicon glyphicon-trash"></i>Remove</button>' +
				'</div>'+
				'</td>' +
				'</tr>';
			if (tableLength > 0) {
	...
	if (orderId) {
		...
		$.ajax({
			...
			success: function (response) {
				...
				// due 
				$("#due").val(response.order[11]);

				// pay amount 
				$("#payAmount").val(response.order[11]);

				var paidAmount = response.order[10]
				var dueAmount = response.order[11];
				var grandTotal = response.order[9];
				...
				$("#updatePaymentOrderBtn").unbind('click').bind('click', function () {
					...
					if (payAmount == "") {
						$("#payAmount").after('<p class="text-danger"> Pay Amount field is required</p>');
